...
// Test 3: Check that the method runs without error when there are no unique
// elements
BOOST_AUTO_TEST_CASE(uniqueArray_test3) {
  ...
  int result[0];
  // Theoretically, it should never copy. If it does, it will go out of bounds
  // in memory.
...
BOOST_AUTO_TEST_CASE(add_test1) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(source2, source2 + 5, source2Cmp,
                                source2Cmp + 5);
}

// ======================= Array Add Non-Destructive Tests
// =================================== Test 2: Test adding with non-destructive
// storage
BOOST_AUTO_TEST_CASE(add_test2) {
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(source1, source1 + 5, source1Cmp,
                                source1Cmp + 5);
  BOOST_CHECK_EQUAL_COLLECTIONS(source2, source2 + 5, source2Cmp,
                                source2Cmp + 5);
}
...
// Test 5: Test finding distinct elements when only first element has no
// duplicate
...
// Test 6: Test finding distinct elements when only last element has no
// duplicate
...
// Test 7: Test finding distinct elements when only first two elements have no
// duplicate
...
// Test 8: Test finding distinct elements when only last two elements have no
// duplicate
...
// Test 2: Test finding distinct elements with count also in arbitrary array
// where all elements have duplicates
...
// Test 3: Test finding distinct elements with count also in arbitrary array
// where no elements has a duplicate
...
// Test 4: Test finding distinct elements with count where first element only
// has no duplicate
...
// Test 5: Test finding distinct elements with count where last element only has
// no duplicate
...
// Test 6: Test finding distinct elements with count where first two elements
// only have no duplicate
...
// Test 7: Test finding distinct elements with count where last two elements
// only have no duplicate
...
BOOST_AUTO_TEST_CASE(minusArray_test1) {
  ...
  cupcfd::error::eCodes err =
      minusArray(source1, 7, source2, 4, &result, &nResult);
...
BOOST_AUTO_TEST_CASE(minusArray_test2) {
  ...
  cupcfd::error::eCodes err =
      minusArray(source1, 7, source2, 8, &result, &nResult);
...
BOOST_AUTO_TEST_CASE(minusArray_test5) {
  ...
  cupcfd::error::eCodes err =
      minusArray(source1, 4, source2, 4, &result, &nResult);
...
BOOST_AUTO_TEST_CASE(
    intersectCount_calculatesCorrectIntersectWithLarger1stArray) {
...
BOOST_AUTO_TEST_CASE(
    intersectCount_calculatesCorrectIntersectWithLarger2ndArray) {
...
BOOST_AUTO_TEST_CASE(intersectCount_returns0WithNoIntersectingElements) {
  int source1[1] = {1};
  int source2[4] = {2, 8, 21, 22};
  int count;
  cupcfd::error::eCodes status;
...
BOOST_AUTO_TEST_CASE(intersectCount_returnsLenWithIdenticalElements) {
  int source1[4] = {2, 8, 21, 22};
  int source2[4] = {2, 8, 21, 22};
  int count;
  cupcfd::error::eCodes status;
...
BOOST_AUTO_TEST_CASE(intersectArray_returnsCorrectIntersectForLargerArray1) {
  int source1[6] = {1, 2, 4, 8, 10, 12};
  int source2[4] = {2, 8, 21, 22};
  ...
  auto status = intersectArray(source1, 6, source2, 4, &result, &nResult);
  BOOST_CHECK_EQUAL(status, cupcfd::error::E_SUCCESS);
...
BOOST_AUTO_TEST_CASE(
    intersectArray_returnsCorrectIntersectWithCommonFinalElements) {
...
BOOST_AUTO_TEST_CASE(intersectArray_returnsEmptyWithNoIntersectingElems) {
  int source1[1] = {1};
  int source2[4] = {2, 8, 21, 22};
...
BOOST_AUTO_TEST_CASE(intersectArray_returnsSameElementsWhenElementsIdentical) {
  int source1[4] = {2, 8, 21, 22};
  int source2[4] = {2, 8, 21, 22};
