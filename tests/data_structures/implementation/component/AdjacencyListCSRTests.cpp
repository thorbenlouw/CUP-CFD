 ...
 * @section DESCRIPTION
 *
 * Unit Tests for the AdjacencyListCSR class
 */

#define BOOST_TEST_MODULE AdjacencyListCSR
...
BOOST_AUTO_TEST_CASE(constructor_test1) { AdjacencyListCSR<int, int> graph; }
...
BOOST_AUTO_TEST_CASE(getEdges_test1) {
  ...
  // Will  keep these sorted since for now getEdges will return in sorted order
  // due to use of map
...
// === existsEdge ===
// Test 1: Determine that edge exists
BOOST_AUTO_TEST_CASE(existsEdge_test1) {
  ...
  int localNodeCmp[3] = {0, 1, 1};
  int adjNodeCmp[1] = {1};  // Local Index of Node 40

  BOOST_CHECK_EQUAL_COLLECTIONS(&(graph.xadj[0]), &(graph.xadj[0]) + 3,
                                localNodeCmp, localNodeCmp + 3);
  BOOST_CHECK_EQUAL_COLLECTIONS(&(graph.adjncy[0]), &(graph.adjncy[0]) + 1,
                                adjNodeCmp, adjNodeCmp + 1);
...
BOOST_AUTO_TEST_CASE(operator_copy_test1) {
  AdjacencyListCSR<int, int> graph;
  AdjacencyListCSR<int, int> graph2;
  cupcfd::error::eCodes status;
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(edges1Cmp, edges1Cmp + 6, edgeNode1Dest,
                                edgeNode1Dest + 6);
  BOOST_CHECK_EQUAL_COLLECTIONS(edges2Cmp, edges2Cmp + 6, edgeNode2Dest,
                                edgeNode2Dest + 6);
}
