...
class CommandLine {
 public:
  ...
  /**
   * Parse the command line inputs to the program and store the options and
   * values.
   *
   ...
   * Convert it to a bool (or return an error if an unsuitable flag was
   * provided)
   *
   * @param opts The object containing the flags + args
   * @param flag The name of the flag the args are associated with
   * @param arg The position of the arg in the arg list of the flag
   * (zero-indexed)
   * @param result Location to store result in
   *
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes getBoolArg(
      std::string flag, int arg, bool* result);
   ...
   *
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes getIntArg(
      std::string flag, int arg, int* result);
   ...
   *
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes getDoubleArg(
      std::string flag, int arg, double* result);
   ...
   * @retval cupcfd::error::E_CMDLINE_ERR_ARG_MISSING_FLAG The flag does not
   * exist in the provided command line options
   * @retval cupcfd::error::E_CMDLINE_ERR_ARG_INVALID_ARG The provided arg index
   * is invalid - it is either negative or there are insufficient arguments to
   * the requested flag
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes getStringArg(
      std::string flag, int arg, std::string* result);
};
}  // namespace cupcfd::structure
