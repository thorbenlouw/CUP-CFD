...
// ToDo: The HDF5 Interface could do with some restructuring - we don't really
...
class HDF5Access {
 public:
   ...
   */
  HDF5Access(const std::string &fileName, HDF5Record& record);
   ...
   *
   * @return An error status indicating the success or failure of the
   * operation
   * @retval cupcfd::error::E_SUCCESS Operation was successful
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes openFile();
   ...
   * @retval cupcfd::error::E_SUCCESS Operation was successful
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes closeFile();
  ...
  /**
   * Read all data stored in the tracked HDF5Record into an array - integer
   * variant.
   *
   ...
   * @retval cupcfd::error::E_SUCCESS Operation was successful
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes readData(
      int* sink) const;

  /**
   * Read all data stored in the tracked HDF5Record into an array - float
   * variant.
   *
   * Records that are 2D or greater will be flattened into a 1D array.
   *
  ...
  /**
   * Read all data stored in the tracked HDF5Record into an array - double
   * variant.
   *
   * Records that are 2D or greater will be flattened into a 1D array.
   *
   ...
   * Records that are 2D or greater will be flattened into a 1D array.
   *
   * This function restricts the data retrieved from the record to those
   * indexes specified in the properties object.
   *
   * @param sink The data array to store the data in.
   ...
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes readData(
      int* sink, HDF5Properties& properties);
   ...
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes readData(
      float* sink, HDF5Properties& properties);
   ...
   */
  __attribute__((warn_unused_result)) cupcfd::error::eCodes readData(
      double* sink, HDF5Properties& properties);
   ...
   * indexes specified in the properties object.
   *
   * This function also creates an appropriately sized array to store the
   * data.
   *
