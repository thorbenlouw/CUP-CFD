...
I uniqueCount(T* source, I nEle) {
  ...
  if (curCount == 1) {
    // The last element was unique since curCount was reset to 1, so increase
    // the overall count
    count++;
  }
...
template <class I, class T>
// TODO: There's no check here for whether dest is big enough to hold the unique
// elements! Switch to std::set
void uniqueArray(T* source, T* dest, I nEle) {
  ...
  for (I i = 1; i < nEle; i++) {
    if (source[i - 1] == source[i]) {
      ...
      // We have changed elements, check if previous element was only
      // encountered once
      if (curCount == 1) {
  ...
  // Check for final element in array
  if (curCount == 1) {
    // The last element was unique since curCount was reset to 1
    dest[ptr] = source[nEle - 1];
  }
}

template <class I, class T>
I distinctCount(T* source, I nEle) {
  // Stores current partial count of how many different values we have
  // encountered
  I count;

  // Could use array::drivers::count, but since that is O(n) this would
  // effectively lead to an O(n^2) if done for every element.

  // Instead, we take advantage of the sorted property to know that when an
  // element changes between indexes, we are encountering a brand new element.
...
void distinctArray(T* source, T* dst, I* dupCount, I nEle) {
  ...
  // Last enocuntered element will be in the dst array, but the count will not
  // be stored (if the last index is unique, then the count for the previous
  // element was stored. If it was the same then the counter was just
  // incremented.) Ensure it is stored here (count should still be preserved
  // from last loop)
  dupCount[ptr] = curCount;
}

// ToDo: This could likely be more efficient (.e.g use a hash to test for
// existance?) This Kernel expects sorted arrays
template <class I, class T>
I minusCount(T* source1, I nSource1, T* source2, I nSource2) {
  ...
  for (I i = 0; i < nSource1; i++) {
    // Will use the kernel to skip the is_sorted error check in the driver since
    // we already expect it to be sorted.
    I index;
    err = cupcfd::utility::kernels::binarySearch(source2, nSource2, source1[i],
                                                 &index);
...
cupcfd::error::eCodes minusArray(T* source1, I nSource1, T* source2, I nSource2,
  ...
  for (I i = 0; i < nSource1; i++) {
    // Will use the kernel to skip the is_sorted error check in the driver since
    // we already expect it to be sorted.
    status = cupcfd::utility::kernels::binarySearch(source2, nSource2,
                                                    source1[i], &index);
...
I intersectCount(T* source1, I nSource1, T* source2, I nSource2) {
  // Loop over whichever array has more elements - this way we cannot miss
  // elements from finishing early and elements not in array1 cannot possibly be
  // in the intersect array, even if in array2
  cupcfd::error::eCodes err;
  ...
  // This kernel assumes that the arrays are sorted and contain only distinct
  // elements
  for (I i = 0; i < sourceCap; i++) {
    // We already expect the kernel to be sorted so we will skip the driver
    // checks and directly use the binary search kernel
    I index;
