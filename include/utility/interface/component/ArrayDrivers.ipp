...
cupcfd::error::eCodes uniqueArray(T *source, I nSourceEle, T **dest,
  ...
  *dest = (T *)malloc(sizeof(T) * *nDestEle);
  ...
  // Pass work along to driver that performs the same function as this, but with
  // the results array set up.
...
template <class I, class T>
cupcfd::error::eCodes uniqueArray(T *source, I nSourceEle, T *dest,
                                  [[maybe_unused]] I nDestEle) {
  cupcfd::error::eCodes status = cupcfd::error::E_SUCCESS;
  bool isSorted;
  isSorted = cupcfd::utility::drivers::is_sorted(source, nSourceEle);

  if (!isSorted) {
    // Non-Destructive Sort
    auto scratch = std::vector<T>(nSourceEle);
    status = cupcfd::utility::drivers::merge_sort(source, scratch.data(),
                                                  nSourceEle);
    CHECK_ECODE(status)
    kernels::uniqueArray(scratch.data(), dest, nSourceEle);
  } else {
...
template <class I, class T>
cupcfd::error::eCodes add(T *source1, I nSource1Ele, T *source2,
                          I nSource2Ele) {
...
template <class I, class T>
cupcfd::error::eCodes distinctArray(T *source, I nEleSource, T *dst,
                                    I nEleDst) {
  cupcfd::error::eCodes status;
  bool sorted;
  T *arrPtr;
...
cupcfd::error::eCodes distinctArray(T *source, I nEleSource, T **dst,
  ...
  *dst = (T *)malloc(sizeof(T) * *nEleDst);
  ...
  // Pass work along to driver that performs the same functionas this, but with
  // the results array set up.
...
cupcfd::error::eCodes distinctArray(T *source, I nEleSource, T *dst, I nEleDst,
  ...
  if (!sorted) {
    ...
    status =
        cupcfd::utility::drivers::merge_sort(source, sourceSorted, nEleSource);
    CHECK_ECODE(status)
  } else {
    sourceSorted = source;
  }
...
template <class I, class T>
cupcfd::error::eCodes distinctArray(T *source, I nEleSource, T **dst,
                                    I *nEleDst, I **dupCount, I *nEleDupCount) {
  cupcfd::error::eCodes status;
  ...
  *dupCount = (I *)malloc(sizeof(I) * *nEleDupCount);
  ...
  // Pass work along to driver that performs the same functionas this, but with
  // the results array set up.
  status = drivers::distinctArray(source, nEleSource, *dst, *nEleDst, *dupCount,
                                  *nEleDupCount);
  CHECK_ECODE(status)

  // Responsibility for freeing the dst and dupCount arrays is left to the
  // caller, since they contain the results.
  return cupcfd::error::E_SUCCESS;
}

template <class I, class T>
// cupcfd::error::eCodes minusCount(T * source1, I nSource1, T * source2, I
// nSource2, I * count) {
void minusCount(T *source1, I nSource1, T *source2, I nSource2, I *count) {
  ...
  *count = cupcfd::utility::kernels::minusCount(source1Ptr, nSource1,
                                                source2Ptr, nSource2);
...
template <class I, class T>
cupcfd::error::eCodes minusDistinctCount(T *source1, I nSource1, T *source2,
                                         I nSource2, I *count) {
  cupcfd::error::eCodes status;
  ...
  *count = 0;
  ...
  // The driver should be non-destructive and return it sorted, so we can reuse
  // it later.
  status =
  ...
  // Call the diff kernel now we are sure we have two sorted arrays.
  *count = cupcfd::utility::kernels::minusCount(
      source1Distinct, nSource1Distinct, source2Distinct, nSource2Distinct);
...
template <class I, class T>
cupcfd::error::eCodes minusArray(T *source1, I nSource1, T *source2, I nSource2,
                                 T **result, I *nResult) {
  T *source1Ptr = source1;
  T *source2Ptr = source2;
  cupcfd::error::eCodes status = error::E_SUCCESS;

  // Error Check: result should be a nullptr (else we risk a memory leak by
  // overwriting an active pointer memory allocation).

  // Kernel requires sorted arrays. Check if source1 is sorted.
  auto sorted1 = cupcfd::utility::drivers::is_sorted(source1, nSource1);
  if (!sorted1) {
  ...
  auto sorted2 = cupcfd::utility::drivers::is_sorted(source2, nSource2);
  if (!sorted2) {
  ...
  // Need to allocate space for the result array, requiring us to know how many
  // elements there are in the distinct array.
  minusCount(source1Ptr, nSource1, source2Ptr, nSource2, nResult);

  *result = (T *)malloc(sizeof(T) * *nResult);

  // Compute the set minus array.
  status = kernels::minusArray(source1Ptr, nSource1, source2Ptr, nSource2,
                               *result, *nResult);
  CHECK_ECODE(status)
...
template <class I, class T>
cupcfd::error::eCodes intersectCount(T *source1, I nSource1, T *source2,
                                     I nSource2, I *count) {
  cupcfd::error::eCodes status;
  ...
  *count = 0;
  ...
  // Kernels requires sorted and distinct arrays (since it's technically a set
  // operation) distinctArray will not only sure it only has unique elements, it
  // will also return an array that is already sorted.
  ...
  // Call the diff kernel now we are sure we have two sorted arrays.
  *count = cupcfd::utility::kernels::intersectCount(source1Ptr, nSource1,
                                                    source2Ptr, nSource2);
...
template <class I, class T>
cupcfd::error::eCodes intersectArray(T *source1, I nSource1, T *source2,
                                     I nSource2, T **result, I *nResult) {
  cupcfd::error::eCodes status;
  ...
  *result = (T *)malloc(sizeof(T) * *nResult);
  ...
  // Compute the set minus array.
  status = kernels::intersectArray(source1Ptr, nSource1, source2Ptr, nSource2,
                                   *result, *nResult);
  CHECK_ECODE(status)
